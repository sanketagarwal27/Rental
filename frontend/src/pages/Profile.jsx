...
const Profile = () => {
  ...
  const handleSendPhoneOtp = async () => {
    ...
    try {
      ...
      if (response?.success) {
        ...
        triggerAlert(
          "success",
          "Verification OTP sent! Check your developer console for the OTP code.",
        );
      }
    } catch (err) {
      triggerAlert(
        "error",
        err.response?.data?.message || "Failed to send SMS OTP",
      );
    } finally {
      setVerifyLoading(false);
    }
  ...
  const handleVerifyOtp = async (e) => {
    ...
    setOtpLoading(true);
    try {
      const response = await verifyPhoneOtp({ otp: otpCode });
      if (response?.success) {
        setUser(response.data);
        setShowOtpModal(false);
        setOtpCode("");
        triggerAlert("success", "Phone number verified successfully!");
      }
    } catch (err) {
      triggerAlert(
        "error",
                ...
                <div className="p-4 rounded-xl bg-zinc-950 border border-zinc-800/80 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  {user?.isVerifiedPhone ? (
                    ...
                    <button
                      ...
                      disabled={verifyLoading}
                      className="px-4 py-2 rounded-xl text-xs font-bold bg-purple-600 hover:bg-purple-500 text-white transition-all shadow-md shadow-purple-600/10 self-start sm:self-auto disabled:opacity-50 cursor-pointer flex items-center gap-1.5"
                    >
                      {verifyLoading && (
                        <Loader2 className="w-3 h-3 animate-spin" />
                      )}{" "}
                    ...
                    <span className="text-xs text-zinc-500 italic self-start sm:self-auto">
