  ...
  const fetchBookings = async () => {
    ...
    try {
      const res = await axiosInstance.get(
        `/admin/bookings?page=${page}&limit=10&search=${search}`,
      );
      if (res.data.success) {
  ...
  const executeCancel = async (otp) => {
    try {
      ...
      const res = await axiosInstance.patch(
        `/admin/bookings/${cancelModal.booking._id}/cancel`,
        payload,
      );
      if (res.data.success) {
            ...
            <form onSubmit={handleCancelSubmit}>
              <div className="p-6 border-b border-zinc-800">
                ...
                <p className="text-sm text-zinc-400 mt-2">
                  You are about to cancel booking {cancelModal.booking?._id}.
                  Please define the exact refund to the customer and payout to
                  the host.
                </p>
              </div>
              <div className="p-6 space-y-4">
                <div>
                  ...
                  <p className="text-xs text-zinc-500 mt-1">
                    Total Paid by Customer: ₹{cancelModal.booking?.amountPaid}
                  </p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-zinc-300 mb-1">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setCancelData({
                        ...cancelData,
                        hostPayout: e.target.value,
                      })
                    }
                    className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4 py-2 focus:border-red-500 focus:outline-none"
                  />
                  <p className="text-xs text-zinc-500 mt-1">
                    Expected Standard Payout: ₹{cancelModal.booking?.hostPayout}
                  </p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-zinc-300 mb-1">
