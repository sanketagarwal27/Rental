...
export default function AdminVehicles() {
  ...
  const fetchVehicles = async () => {
    ...
    try {
      const res = await axiosInstance.get(
        `/admin/vehicles?page=${page}&limit=10&search=${search}`,
      );
      if (res.data.success) {
  ...
  const executeBlock = async (otp) => {
    try {
      ...
      const res = await axiosInstance.patch(`/admin/vehicles/${id}/block`, {
        otp,
      });
      if (res.data.success) {
        toast.success(res.data.message);
        setVehicles((prev) =>
          prev.map((v) => (v._id === id ? { ...v, isBlocked: !isBlocked } : v)),
        );
        setOtpModal({ open: false });
      }
    } catch (error) {
      toast.error(
        error.response?.data?.message || "Failed to block/unblock vehicle",
      );
      throw error;
    }
  };
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden flex flex-col h-175">
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex flex-col gap-1 items-start">
                      <span
                        className={`px-2 py-0.5 rounded text-[10px] font-medium uppercase tracking-wider ${v.status === "Approved" ? "bg-emerald-500/10 text-emerald-400" : v.status === "Pending" ? "bg-amber-500/10 text-amber-400" : "bg-zinc-800 text-zinc-400"}`}
                      >
                        {v.status}
                      </span>
                      {v.isBlocked && (
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm">
                    <div className="flex justify-end gap-2">
                      ...
                      <button
                        ...
                        title={
                          v.isBlocked ? "Unblock Vehicle" : "Block Vehicle"
                        }
                      >
                        {v.isBlocked ? (
                          <CheckCircle size={18} />
